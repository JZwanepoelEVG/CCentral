...
import { useSidebar, useThemeStore } from "../../../themeStore";
...
//-- duplicate of your fetchActiveTimer fn from ActiveTimer.tsx ---
async function fetchActiveTimer() {
    const res = await fetch("/api/timer/active");
    if (!res.ok) return null;
    const { success, timer } = await res.json();
...
const NavTools = ({ isDesktop, isMobile, sidebarType }: { isDesktop: boolean; isMobile: boolean; sidebarType: string }) => {
      // subscribe to the same query key so we know if a timer is running
    const { data: activeTimer, isFetched } = useQuery({
